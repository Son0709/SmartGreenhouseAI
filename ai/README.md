...
│   └── 03b_train_leaf_augmented.ipynb       # train lại nhánh lá với augmentation màu/ánh sáng tăng cường, so trực tiếp với baseline (đã có)
├── datasets/
...
12. **`03b_train_leaf_augmented.ipynb`** (thử nghiệm cải thiện nhánh lá, cùng ý tưởng với 8c) — Add Input dataset `tomato_leaf_disease_v1` mới nhất (đã loại 62 ảnh gắn nhãn sai). Train lại YOLOv8n với `hsv_s`/`hsv_v`/`hsv_h` cao hơn mặc định, so trực tiếp với baseline (đã lưu cứng) trên metrics tổng thể, AP50 từng class (đặc biệt `leaf_mold`, class yếu nhất), và tỷ lệ báo động giả trên ảnh negative (baseline 19,7%). Đây là thử nghiệm — không chắc chắn có ích, notebook tự đưa ra kết luận dựa trên số đo thật.
...
**Nhánh bệnh lá (`tomato_leaf_disease_v1`, YOLOv8n baseline):** đạt cả 3 mục tiêu MVP thoải mái (Precision 0,918 / Recall 0,872 / mAP@0.5 0,937) sau 1 vòng cải thiện dữ liệu. `01c_review_negative_leaf.ipynb` phát hiện một lô ảnh liên tiếp trong dữ liệu gốc Roboflow bị gắn nhãn Healthy sai (`Leaf-Mold-509`–`524`, `Late-Blight-650`–`654`); loại 62 ảnh đã xác nhận và train lại giúp `leaf_mold` Recall tăng từ 0,666 → 0,774, tỷ lệ báo động giả trên ảnh lá khỏe giảm từ 24,6% → 19,7%. Điểm yếu còn lại: `leaf_mold` vẫn là class thấp nhất (AP50 0,876 so với 0,93–0,97 của 3 class kia), và báo động giả 19,7% vẫn sát ngưỡng cảnh báo 20%.

**Đang thử nghiệm — `03b_train_leaf_augmented.ipynb`:** áp dụng cùng công thức augmentation màu/ánh sáng đã hiệu quả bất ngờ ở nhánh độ chín (`hsv_s` 0,7→0,9, `hsv_v` 0,4→0,6, `hsv_h` 0,015→0,02), thử xem có giảm được báo động giả + cải thiện `leaf_mold` không. *(Chưa có kết quả thật — cập nhật số liệu vào đây sau khi chạy trên Kaggle.)*
...
1. **Ưu tiên cao nhất — thu thập ảnh thật từ camera IMX179** trong đúng môi trường nhà kính triển khai (cả quả lẫn lá). Đây là hành động ngoài phạm vi notebook (cần phần cứng thật), nhưng là nút thắt quan trọng nhất: domain shift đã xác nhận ở nhánh độ chín không thể sửa bằng cách xử lý lại dữ liệu public hiện có — chỉ dữ liệu thật mới thu hẹp được gap này. Ảnh thật cũng nên dùng làm test set thực tế thay thế dần cho `test_outdomain_openfield`.
   - **Nhân tiện đợt thu thập này, bổ sung luôn class thứ 4 cho nhánh độ chín: `fruit_damaged` ("quả bị hư")**, theo yêu cầu người dùng. Đã cân nhắc dùng tạm `enalis/tomatoes-dataset` (Kaggle, gán nhãn Old/Damaged) nhưng **quyết định không dùng** vì đó là dataset classification (box phải bao trùm cả ảnh) — cùng vấn đề đã gặp với `leaf_zenodo`, dễ làm hỏng chất lượng định vị box của 3 class hiện có nếu gộp thẳng. Chờ ảnh thật có box cục bộ đúng chuẩn thay vì làm 2 lần.
